...
export async function loader() {
    ...
    const publicUrl = supabase.storage.from('bendocs_images').getPublicUrl('projects_images/loleaf_image_1.png')
    ...
    console.log(projects[1].description)
            ...
            <div className="flex flex-row sticky z-50 lg:z-10 lg:static justify-center items-center w-full lg:min-h-24 h-fit p-3 space-x-4 border-b rounded-none">
                {
                    data.projects.map((currentProject, index) => (
                        <ProjectButton key={index} title={currentProject.name} id={currentProject.id} project={currentProject} setProject={setSelectedProject} />
                    ))
                }
            </div>
                ...
                <div className="flex justify-center items-center w-full h-full rounded-none">
                    <div className='flex flex-col w-4/5 lg:w-1/2 justify-center items-center rounded-none'>
                        <div className="flex flex-col flex-grow px-10  justify-start items-center  space-y-4  lg:min-w-[550px] my-10 ">
                            ...
                            <div className="flex flex-row space-x-4">
                                {
                                    project.stack.map((technology, index) => (
                                        <Badge key={index} variant={"secondary"}>{technology}</Badge>
                                    ))
                                }
                            </div>
                            ...
                            <div className="flex flex-row space-x-4">
                                {
                                    project.link && <a href={project.link} target='_blank'><Badge> <LinkIcon className='bg-transparent h-4 w-4 ' color='black' /> Voir le projet </Badge>  </a>
                                }

                                {
                                    project.source_link && <a href={project.source_link} target='_blank'><Badge> <LinkIcon className='bg-transparent h-4 w-4 ' color="black" /> Voir les sources </Badge>  </a>
                                }
                        ...
                        <div className="flex flex-col  justify-center items-center  lg:w-full ">
                                    ...
                                    <CarouselContent className='h-96 max-w-prose'>
                                        {
                                            project.images.map((image, index) => (
                                                ...
                                                <CarouselItem key={index} className='h-96 flex items-center'  >

                                                    <img src={`https://oplyzkzywrzqngstylak.supabase.co/storage/v1/object/public/bendocs_images/projects_images/${image}.png`} alt="" className="rounded-lg object-cover" />
                                                </CarouselItem>
                                            ))
                                        }
                                    </CarouselContent>
                            ...
                            <p className='lg:max-w-prose pb-3 w-full text-lg font-medium tracking-tight rounded-none text-justify' dangerouslySetInnerHTML={{ __html: marked(project.description) }}>
